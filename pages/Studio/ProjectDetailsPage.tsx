...
import { StudioProject, StudioSession } from '../../types/studio';
...
export function StudioProjectDetailsPage() {
  ...
  const { studioProjects, updateStudioProject } = useApp() as any;
  ...
  const project = studioProjects.find((p: any) => p.id === projectId) || MOCK_PROJECT;
  ...
  const handleAddAllToShoppingList = (sessionId: string) => {
    ...
    const sessionProductIds = session.recommendations.map((r: any) => r.id);
